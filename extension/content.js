// Brainrot Filter — WebExtension content script
// Ported from brainrot.user.js. No GM_ APIs; uses fetch() and browser.runtime.sendMessage.

let SERVER = "http://localhost:8787"; // overwritten on first fetchConfig() call with app's setting
...
async function fetchConfig() {
    try {
        // Ask background script — it tries native app first, then server, then defaults
        const config = await browser.runtime.sendMessage({ type: "getConfig", serverUrl: SERVER });
        if (config?.serverURL) SERVER = config.serverURL; // update server URL from app settings
        return config || DEFAULT_CONFIG;
    } catch (_) {
        // Background unavailable (e.g. extension disabled mid-session) — hit server directly
        try {
...
// contentId — unique cache key (prefixed per platform)
...
// Instagram uses dir="auto" on all user-generated text — stable across class name obfuscation
function extractInstagramText(searchRoot) {
    const candidates = Array.from(searchRoot.querySelectorAll('[dir="auto"]'))
...
// Proximity-based caption extraction for the Instagram feed (no article wrapper)
...
function getFacebookDetails() {
    ...
    const title = titleEl?.textContent?.trim() || document.title.replace(/ \| Facebook$/, "").trim();
...
function startHold(vid) {
    ...
    // Re-enforce every 200ms — players fight back against pause/style changes
...
// ── Non-YouTube Phase 2: score description/caption text via /ingest ───────────
    ...
    async function callPlayer(clientLabel, clientName, clientVersion, clientNumber, credMode) {
        ...
        try {
            const res = await fetch(
                `https://www.youtube.com/youtubei/v1/player?key=${encodeURIComponent(ctx.apiKey)}&prettyPrint=false`,
                {
                    method: "POST",
...
async function run() {
    ...
    // Phase 1: instant title keyword score
    ...
    const intersectionObserver = new IntersectionObserver((entries) => {
        for (const entry of entries) {
            ...
            debounceTimer = setTimeout(
                () => scoreVisibleVideo(vid).catch(e => log("feed score error:", e.message)),
                600
            );
            break;
        }
